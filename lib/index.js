...
/**
 * @typedef HandlebarsOptions
 * @type {object}
 * @property {object} [templateContext]            Additional context to be passed to the template.
 * @property {boolean} [expandLicenseFileContents] Controls whether to include the license file's content or not.
 */

/**
 * Exports data using a custom Handlebars template.
 * @method
 * @param  {JSON} sorted                        The sorted JSON data from all packages.
 * @param  {String} templatePath                The handlebars template to use.
 * @param  {HandlebarsOptions} [handlebarsOpts] Options for handlebars.
 * @return {String|Error}                       The returning plain text.
 */
exports.asHandlebars = function(sorted, templatePath, handlebarsOpts) {
    ...
    try {
        const jsonFileContents = fs.readFileSync(templatePath, { encoding: 'utf8' });
        const handlebars = require('handlebars');
        const template = handlebars.compile(jsonFileContents);
        const depContext = {
            dependencies: Object.keys(sorted).map((key) => {
                const result = sorted[key];
                result.title = key.slice(0, key.lastIndexOf('@'));
                result.version = key.substring(key.lastIndexOf('@') + 1);

                if (handlebarsOpts.expandLicenseFileContents && result.licenseFile) {
                    result.licenseFileContent = fs.readFileSync(result.licenseFile, { encoding: 'utf8' });
                }

                return result;
            })
        };
        context = Object.assign(context, depContext);
        context = Object.assign(context, handlebarsOpts.templateContext);
